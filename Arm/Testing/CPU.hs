...
flushPipeline = do (CPU _ _ _ _ aux) <- get
                   case aux of 
                     Nil -> return ()
                     (InO fd de em ew) -> setAuxilary (InO [] [] em ew)
...
instrsExecuted :: (MonadState CPU m, MonadIO m) => m Integer
...
decodeFail :: (MonadState CPU m, MonadIO m) => m ()
decodeFail = do f <- getCounter "DecodeFailures"
                setCounter "DecodeFailures" (f + 1)
                
failedDecodes :: (MonadState CPU m, MonadIO m) => m Integer
failedDecodes = getCounter "DecodeFailures"
